...
template <typename X, typename Y> struct Sum
{
    ...
    typedef typename Simpl< Fraction<Num, Den> >::result result;
};

template <int N> struct Fractal
{
    static const long result = N * Fractal < N - 1 >::result;
};
template <> struct Fractal<0>
{
...
{
    // e = S(1/n!) = 1/0! + 1/1! + 1/2! + ...
    static const long Den = Fractal<N>::result;
    typedef Fraction< 1, Den > term;
    typedef typename E < N - 1 >::result next_term;
    typedef typename Sum< term, next_term >::result result;
};
template <> struct E<0>
{
    typedef Fraction<1, 1> result;
};
